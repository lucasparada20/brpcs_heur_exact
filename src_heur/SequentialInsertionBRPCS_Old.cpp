...
void SeqInsertBRPCS::FillPathWithPos(Sol & s, Driver* d, Node* n, int pos, int & init_q, int & init_qe, bool & has_zero_rec) {
    path.clear();
...
void SeqInsertBRPCS::Insert(Sol & s, bool show)
{
	...
	for (size_t i = 0; i < nodes.size(); i++) 
	{
		...
		{	
			...
			FillPathWithPos(s, d, n, move.pos, init_q, init_qe , has_zero_first_pass); // fills std::vector<Node*> path
			...
			if (cost < 9999.0) {
				...
				for (int l = 1; l <= nb_look_ahead; ++l) {
					if (k + l < move_vec.size()) {
						...
						FillPathWithPos(s, nextDriver, n, next_move.pos, init_q, init_qe, has_zero_first_pass);
						
						double nextRec = 0.0;
						if(Parameters::GetCostPolicy()) // 1 == Restock
						{
							if(Parameters::GetBSSType() == CS && has_zero_first_pass){
								has_zero_rec = RouteFeasibility::HasZeroHCUncharged(path,Q,false,init_q,init_qe);
							} else if (Parameters::GetBSSType() == SW && has_zero_first_pass){
