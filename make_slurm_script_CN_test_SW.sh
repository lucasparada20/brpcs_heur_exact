...
for heur in "${CONSTRUCTION_HEURISTIC[@]}"; do
	...
	for file in "$INSTANCE_DIR"/*_SW.txt; do
		if [[ -f "$file" ]]; then
			...
			re_file="re_CN_${heur}_${city}_20_SW.txt" # Expected result file
...
build/exec_heur instance_file=$file construction_heuristic=$heur > output/${heur}_${base_name}.out 2> output/${heur}_${base_name}.err
EOF
