...
MAX_DISTANCES=("75" "70" "65" "60" "55" "50")
CITIES_TO_LAUNCH=("mexicocity" "montreal" "quebec" "rio" "washington")
	...
	for heur in "${CONSTRUCTION_HEURISTIC[@]}"; do
		...
		for file in "$INSTANCE_DIR"/*_CS.txt; do
			if [[ -f "$file" ]]; then
				...
				re_file="re_CN_${heur}_${dist}_${city}_20_CS.txt" # Expected result file
