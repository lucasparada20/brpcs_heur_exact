...
for heur in "${CONSTRUCTION_HEURISTIC[@]}"; do
	...
	for file in "$INSTANCE_DIR"/*_SW.txt; do
		if [[ -f "$file" ]]; then
			...
			re_file="re_RT_${heur}_${city}_20_SW.txt" # Expected result file
...
build/exec_heur instance_file=$file construction_heuristic=$heur initial_solution_file=solutions/solution_CN_${heur}_${city}_20_SW.txt > output/RT_${heur}_${base_name}.out 2> output/RT_${heur}_${base_name}.err
EOF
